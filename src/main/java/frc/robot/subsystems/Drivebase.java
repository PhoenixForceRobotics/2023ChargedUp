...
public class Drivebase extends SubsystemBase {
    ...
    private final GenericEntry currentXVelocityEntry, currentYVelocityEntry, currentRotationalVelocityEntry; // entries for telemetry

    public Drivebase()
    {
        // creates the components on the drivebase
        flWheel = new Motor(DrivebaseConstants.WHEEL_FL_PORT, DrivebaseConstants.WHEEL_FL_REVERSED, DrivebaseConstants.GEAR_RATIO, DrivebaseConstants.WHEEL_DIAMETER);
        frWheel = new Motor(DrivebaseConstants.WHEEL_FR_PORT, DrivebaseConstants.WHEEL_FR_REVERSED, DrivebaseConstants.GEAR_RATIO, DrivebaseConstants.WHEEL_DIAMETER);
        blWheel = new Motor(DrivebaseConstants.WHEEL_BL_PORT, DrivebaseConstants.WHEEL_BL_REVERSED, DrivebaseConstants.GEAR_RATIO, DrivebaseConstants.WHEEL_DIAMETER);
        brWheel = new Motor(DrivebaseConstants.WHEEL_BR_PORT, DrivebaseConstants.WHEEL_BR_REVERSED, DrivebaseConstants.GEAR_RATIO, DrivebaseConstants.WHEEL_DIAMETER);

        flPiston = new DoubleSolenoid(PneumaticsModuleType.REVPH, DrivebaseConstants.FL_BUTTERFLY_FORWARD_PORT, DrivebaseConstants.FL_BUTTERFLY_REVERSE_PORT);
        frPiston = new DoubleSolenoid(PneumaticsModuleType.REVPH, DrivebaseConstants.FR_BUTTERFLY_FORWARD_PORT, DrivebaseConstants.FR_BUTTERFLY_REVERSE_PORT);
        blPiston = new DoubleSolenoid(PneumaticsModuleType.REVPH, DrivebaseConstants.BL_BUTTERFLY_FORWARD_PORT, DrivebaseConstants.BL_BUTTERFLY_REVERSE_PORT);
        brPiston = new DoubleSolenoid(PneumaticsModuleType.REVPH, DrivebaseConstants.BR_BUTTERFLY_FORWARD_PORT, DrivebaseConstants.BR_BUTTERFLY_REVERSE_PORT);
        
        gyro = new ADXRS450_Gyro();

        // Sets the current wheel positions
        currentWheelPositions = new MecanumDriveWheelPositions(
            flWheel.getMeters(),
            frWheel.getMeters(),
            blWheel.getMeters(),
            brWheel.getMeters()
        ...
        currentWheelSpeeds = new MecanumDriveWheelSpeeds(); // the velocities of each wheel (not just "speed" :/)
        desiredWheelSpeeds = new MecanumDriveWheelSpeeds(); // the *velocities* being sent to pid controllers
        currentChassisSpeeds = new ChassisSpeeds(); // the VELOCITIES of the robot relative to the robot
        desiredChassisSpeeds = new ChassisSpeeds(); // The ***VELOCITY*** we want to the set the robot to 
        // (WPI needs to work on their language and correct terminology)
        ...
        currentRotationalVelocityEntry = drivebaseTab.add("Current Rotational Velocity", 0).getEntry();
    }
    ...
    public void periodic() {
        ...
        if(!isMeccanum)
        {
            ...
            centerOfRotation = CenterOfRotation.CENTER; // Causes the math to work like standard differential drive
        }
        ...
        // Update then set the (estimated) pose from odometry (not very accurate, reset odometry often)
        ...
        // Scales the values to prevent values from being too high (100%, velocity -> output of motor)
        ...
        currentRotationalVelocityEntry.setDouble(currentChassisSpeeds.omegaRadiansPerSecond * 180 / Math.PI);
