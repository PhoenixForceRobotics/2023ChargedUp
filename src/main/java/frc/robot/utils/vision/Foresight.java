...
import frc.robot.utils.vision.Foresight.SlotPriority; // java isn't mad about cyclical imports for

// interfaces, apparently
...
/**
 * Class for optimizing piece placement. Also contains a handy journal of my descent into madness
 * why not TODO: rewrite functions for actually pulling best slots for Speed:tm:
 */
public class Foresight {
    ...
    private static SlotPriority[][][]
            gridSlotPriorities; // 0: highest scoring (obvious), 1: least positional movement
    // (collision avoidance), 2: least positional/arm movement (cycle
    // times)
    ...
    /**
     * Overwrites grid. I can make these comments as bad as I want and nobody can stop me
    ...
    /**
     * Pokes value into grid at given coordinates. GCS (grid coordinate system and def not gender
     * confirmation surgery) is as follows: (0, 0) is at the bottom left hybrid node; (8, 2) is at
     * the top right cone node
     */
    ...
    public static SlotPriority[][] getSlotPriorities(PlacementStrategy strategy) {
    ...
    /**
     * generic evaluator just so I don't forget to separately copy and paste however many god
     * forsaken copies of this I'm gonna need once it inevitably breaks
     */
    private static SlotPriority[][] calculateSlotPriorities(
            EvaluationFunction eval, int robotSlotX, int robotSlotY) {
        // TODO: optimize this
    ...
    public static SlotPriority[][] calculateSlotPriorities(
            PlacementStrategy strategy, int currentGridX, int currentGridY) {
        int which;
        switch (strategy) {
            case MAXIMIZE_SCORE:
            ...
            case MINIMIZE_MOVEMENT:
                ...
                break;
            case MINIMIZE_TIME:
                Foresight.calculateSlotPrioritiesMinTime(currentGridX, currentGridY);
                which = 2;
                break;
            default:
                Foresight.calculateSlotPrioritiesMaxScore();
    ...
    public static void calculateSlotPriorities(int currentGridX, int currentGridY) {
        Foresight.gridSlotPriorities[0] = Foresight.calculateSlotPrioritiesMaxScore();
    ...
    /**
     * Try to make each placement as impactful as possible. We don't care if we hit people or have
     * slower cycles, we want to win on quality by any means necessary.
     */
    private static SlotPriority[][] calculateSlotPrioritiesMaxScore() {
        ...
        EvaluationFunction eval =
                (int peak,
                        ...
                        int robotSlotY) -> {
                    SlotPriority out;
                    if (Foresight.gridSlots[slotX][
                            slotY]) { // ignore basically everything if it's already filled
    ...
    /**
     * Try to move the robot as little as possible. We don't care if we move the arm a lot, we just
     * don't want to hit people.
     */
    private static SlotPriority[][] calculateSlotPrioritiesMinMovement(int currentGridX) {
        // I'm not repasting my earlier comments literally just scroll up most of this is copy
        // pasted
        EvaluationFunction eval =
                (int peak,
                        int slotX,
    ...
    /**
     * Try to move both robot and arm as little as possible while still scoring as much as possible.
     * We want to drop cycle time as low as it'll go and win on quantity.
     */
    private static SlotPriority[][] calculateSlotPrioritiesMinTime(
        ...
        return Foresight.calculateSlotPriorities(eval, currentGridX, currentGridY);
    }

    /**
     * Returns the adjacency of a row; i.e. for each slot, how much would placing a piece here
     * contribute to a link?.
     */
    private static int[] calculateAdjacency(int row) {
        ...
        for (int slot = 0; slot < 9; slot++) {
            ...
            for (int offset = -2; offset < 3; offset++) {
                if (VisionMath.clamp(slot + offset, 0, 8)
                        == slot + offset) { // ensure checked slot exists
                    if (slot + offset
                            != slot) { // ensure we're not counting a slot towards its own adjacency
        ...
        // adjacency link resetter (to avoid the algorithm trying to count pieces for multiple
        // links)
        for (int slot = 0; slot < 7; slot++) {
            if (Foresight.gridSlots[row][slot]
                ...
                output[slot + 1] -= 2;
                output[slot + 2] -= 2;
                slot += 3; // ensure we don't decrement stuff unnecessarily (e.g. a 4 in a row
                // cutting the adjacency of the center 2 twice)
            }
        }
    ...
    /**
     * Returns the adjacency of the grid; for each slot within the grid, how much would placing a
     * piece here contribute to a link?
     */
    ...
    private static int calculatePiecesInCoopGrid() {
        ...
        for (int x = 3; x < 6; x++) {
            for (int y = 0; y < 3; y++) {
                output +=
                        Foresight.gridSlots[x][y]
                                ? 1
                                : 0; // ternary operators are proof that god is dead
            }
        }
        return output;
    }
