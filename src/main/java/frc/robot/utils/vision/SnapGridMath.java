...
public class SnapGridMath {
    ...
    public static int snapToGrid(
        ...
        double substationXDistance = Math.abs(
            xPosition - SnapGrid.SUBSTATION_SNAP_X
        );
        ...
        for (int i = 0; i < SnapGrid.GRID_SNAP_Y.length; i++) {
            double yDifference = Math.abs(
                yPosition - (SnapGrid.GRID_SNAP_Y[i] + teamYOffset)
            );
        ...
        return (
                (
                    (
                        Math.pow(bestYDistanceValue, 2) +
                        Math.pow(gridXDistance, 2)
                    )
                ) <
    ...
    public static int snapToGrid(Alliance alliance, Translation2d position)
        ...
        return snapToGrid(
            alliance,
            position,
            SnapGrid.DEFAULT_SNAP_MAX_DISTANCE
        );
    }
     ...
     */
    public static Pose2d getSnapPositionFromPosition(
        Alliance alliance,
        Pose2d positionPose
    )
    ...
    public static Pose2d getSnapPositionFromIndex(
        Alliance alliance,
        Pose2d positionPose,
        int index
    )
