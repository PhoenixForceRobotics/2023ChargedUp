...
public class SetAngle extends CommandBase {
    ...
    public SetAngle(Arm arm, double desiredAngle) {
        ...
        pidController =
                new PIDController(
                        ...
                        ArmConstants.ROTATION_PID_D); // This will keep the motors from
        // overshooting/undershooting

        rotationTab =
                Shuffleboard.getTab(
                        "Rotation"); // This tab will be used for any data that needs to be
        // displayed on the shuffleboard
        angleError =
                rotationTab
                        .add("Angle Error", 0)
                        .getEntry(); // Displays the error (the difference between the desired angle
        // and the current angle)
        currentAngle =
                rotationTab
                        .add("Current Angle", 0)
                        .getEntry(); // Displays the current angle of the arm
        rotationTab.add(
                "PID Controller",
                pidController); // Displays the pid controller values on the shuffleboard
    }
    ...
    @Override
    public void execute() {
        // Calculates the output that needs to be sent in radians per second to the motors by using
        ...
        // Calculates the error of the arm and sets the shuffleboard data to that value to be
        // displayed
    ...
    public void end(boolean interrupted) {
        arm.setRotationRadiansPerSecond(
                0); // Once the command is done, it will stop the motors so that we won't have any
        // excess movement
    }
}
