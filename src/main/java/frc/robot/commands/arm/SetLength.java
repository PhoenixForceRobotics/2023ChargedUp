...
public class SetLength extends CommandBase {
     ...
     * @param desiredLength - the length that the arm will be set to in meters (RELATIVE TO CENTER
     *     OF ROTATION)
     */
    public SetLength(Arm arm, double desiredLength) {
        ...
        firstStageController =
                new PFRSimplePIDController(ArmConstants.FIRST_STAGE_LENGTH_PID_VALUES);
        firstStageController.setTolerance(
                ArmConstants.ROTATIONAL_SETPOINT_ERROR.getFirst(),
                ArmConstants.ROTATIONAL_SETPOINT_ERROR.getSecond());

        secondStageController =
                new PFRSimplePIDController(ArmConstants.SECOND_STAGE_LENGTH_PID_VALUES);
        firstStageController.setTolerance(
                ArmConstants.ROTATIONAL_SETPOINT_ERROR.getFirst(),
                ArmConstants.ROTATIONAL_SETPOINT_ERROR.getSecond());

        double proportionOfTotalLength =
                desiredLength
                        - ArmConstants.NO_EXTENSION_LENGTH
                                / (ArmConstants.FULL_EXTENSION_LENGTH
                                        - ArmConstants.NO_EXTENSION_LENGTH);
        double desiredFirstStageLength =
                proportionOfTotalLength * ArmConstants.FIRST_STAGE_MAX_EXTENSION;
        double desiredSecondStageLength =
                proportionOfTotalLength * ArmConstants.SECOND_STAGE_MAX_EXTENSION;

        firstStageController.setSetpoint(desiredFirstStageLength);
        secondStageController.setSetpoint(desiredSecondStageLength);

        firstStageController.setTolerance(
                ArmConstants.EXTENSION_SETPOINT_ERROR.getFirst(),
                ArmConstants.EXTENSION_SETPOINT_ERROR.getSecond());
    }
    ...
    public boolean isFinished() {
        return (firstStageController.atSetpoint()
                && secondStageController
                        .atSetpoint()); // Checks if the arm is at the desired length
    }
